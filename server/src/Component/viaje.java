    ...
    public static void getActivo(JSONObject obj, SSSessionAbstract session) {
        try {
            ...
            if(obj.has("key_usuario")){
                data = SPG.single_object(VIEW, "estado > 0", "key_usuario = '" + obj.getString("key_usuario") + "'");
            }

            if(obj.has("key_conductor")){
                data = SPG.single_object(VIEW, "estado > 0", "key_conductor = '" + obj.getString("key_conductor") + "'");
            }
