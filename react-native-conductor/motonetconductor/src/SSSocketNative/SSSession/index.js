...
class SSSession {
    ...
    constructor(config, store, insertInReducer) {
        ...
        this.colaMensaje = new SSColaMensaje(this);
    ...
    open() {
        if (this.isActivo()) {
            Log("YA SE ENCUENTRA CONECTADO " + this.config["nombre"] + "@" + this.config["ip"] + ":" + this.config["puerto"] + " ::open");
        ...
        var end_cert = "\n-----END CERTIFICATE-----";
        var cerdata = this.config["cert"];
        var pem = cert_begin + cerdata + end_cert;
        Log("Itentando conectar con " + this.config["nombre"] + "@" + this.config["ip"] + ":" + this.config["puerto"] + " ::open");
        ...
        const reintent = async () => {
            ...
            if (!_instance.identificado) {
                ...
                if (curTime - _instance.lastSendIdentificado > 3000) {
                    ...
                    AsyncStorage.getItem(AppParams.storage.usuarioLog).then((strUsuario) => {
