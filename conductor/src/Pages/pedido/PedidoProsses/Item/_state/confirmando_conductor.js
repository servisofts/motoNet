...
import { SButtom, SDate, SHr, SIcon, SImage, SInput, SList, SLoad, SLocation, SMapView, SMath, SNavigation, SPage, SPopup, SText, STheme, SView } from 'servisofts-component';
...
export default class confirmando_conductor extends Component {
        ...
        return <SView col={"xs-12"} row center >
            <SView col={"xs-5.5"} padding={4} style={{ alignItems: "flex-end" }}>
                <SView width={120} height={35} center
                    ...
                    {this.state.loading ? <SLoad /> : <SText bold color={STheme.color.white}>RECHAZAR</SText>}
                </SView>
            ...
            <SView col={"xs-5.5"} padding={4} style={{ alignItems: "flex-start" }} >
                <SView width={120} height={35} center
                    ...
                    {this.state.loading ? <SLoad /> : <SText bold color={STheme.color.white}>CONFIRMAR</SText>}
    ...
    render() {
        ...
        if (this.props?.parent?.enviroments) {
            env = parseFloat(this.props.parent.enviroments["tiempo_de_espera_confirmando_conductor"]?.value ?? 30)
        }

        if (data.state == "confirmando_conductor") {
            let time = new SDate(data?.fecha_edit, "yyyy-MM-ddThh:mm:ss").diffTime(new SDate());
            if (time > 30000) return null;
        }
        let usuario = Model.usuario.Action.getByKey(data.key_usuario) ?? {};
        return (<SView col={"xs-12"} center style={{ overflow: "hidden", borderRadius: 8 }}
            backgroundColor={STheme.color.accent}
        // backgroundColor={this.uuidToColor(data.key)}
