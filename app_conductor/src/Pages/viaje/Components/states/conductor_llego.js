...
export default class conductor_llego extends Component {
    ...
    renderCurrentDestino = (currentDestino) => {
        return <SView col={"xs-12"} center flex>
            <SText>COMPLETASTE LOS DESTINOS</SText>
            <SButtom type='outline' onPress={() => {
                Model.viaje.Action.action("fin_viaje", this.props.viaje.key).then((resp) => {
                    SNavigation.reset("/");
                })
            }}>FINALIZAR VIAJE</SButtom>
        </SView>
    }
    render() {
        const currentDestino = this.getCurrentDestino();
        if (!currentDestino.key) {
            return this.renderCurrentDestino(currentDestino)
        }
        return (
            <SPage hidden header={<TopBar type={"usuario"} />}>
                ...
                <SView col={"xs-12"} padding={8} center>
                    <SHr h={16} />
                    <SText bold fontSize={18}>CONFIRMA PARA IR AL SIGUIENTE DESTINO</SText>
                    ...
                    <SView row col={"xs-12"}>
                        ...
                        <SView flex />
                        <SButtom type='outline' onPress={() => {
                            Model.viaje.Action.action("inicio_viaje", this.props.viaje.key, {
