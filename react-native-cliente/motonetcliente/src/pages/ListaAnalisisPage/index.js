...
export class ListaAnalisisPage extends Component {
    ...
    componentDidMount() {
        if (!this.props.state.analisisReducer.data) {
            this.getAllLista();
        }
    }
    ...
    getAllLista = () => {
        console.log("Obtengo datos de servidor analisis")
    ...
    getLista = () => {
        if (this.props.state.analisisReducer.estado == "cargando" && this.props.state.analisisReducer.type == "getAllByUsuario") {
            // console.log("entroo")
            return (
                <View style={{
                    flex: 1,
        ...
        if (!this.props.state.analisisReducer.data) {
            this.getAllLista();
            // console.log("analisisss")
            return <View />
        }
        var data = this.props.state.analisisReducer.data;
        ...
        return (
            <FlatList
                ...
                renderItem={({ item }) => {
                    var obj = data[item];
                    var url = urlFoto.urlImages + "analisis_small.png" + `?type=analisis&key=${item}`;
                    var urlLarge = urlFoto.urlImages + obj.foto + `?type=analisis&key=${item}`;
        ...
        return (
            <View style={{
                ...
                {/* <TouchableOpacity
                    ...
                    onPress={() => {
                        // alert("dfdf")
                        this.props.navigation.navigate("SubirAnalisisPage");
                    }}>
                    <Text style={{
                        color: "#fff"
                    }}>
...
export default connect(initStates)(ListaAnalisisPage);
